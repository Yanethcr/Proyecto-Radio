...
(function () {
  ...
  function reproducir(urlOriginal, nombre, lugar, btn) {
    ...
    // Error de red (stream caído, ERR_CONNECTION_CLOSED, etc.)
    aud.onerror = () => {
      if (btn) btn.innerHTML = '<i class="fas fa-play"></i>';
      const elNombre = document.getElementById('rep-nombre');
      ...
      if (typeof window.alert === 'function') {
        alert('Esta estación no está disponible en este momento. Prueba con otra.');
      }
    };
    ...
    // Resetear corazón al cambiar de estación
  ...
  document.querySelector('.icono-btn[title="Favorito"]').addEventListener('click', async function () {
    ...
    if (icono.style.color === 'rgb(28, 240, 12)') { alert("Ya está en favoritos."); return; }
  ...
  const ATLAS_URLS = [
    ...
    'https://rawcdn.githack.com/topojson/world-atlas/master/countries-110m.json'
  ];
  ...
  // ── CORRECCIÓN 1: Cerrar panel sin pausar el audio ──
