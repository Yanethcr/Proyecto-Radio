/* =============================================
   AUDIO TRAVELER — globo.js v2 Unificado (Corregido + DB Connect)
   ============================================= */
...
// ── MEJORA CORREGIDA: Manejo avanzado de promesa de audio ──
    ...
    if (promesa !== undefined) {
      promesa.catch(() => {
        ...
        // Usamos la función global del mensaje flotante
        if (typeof mostrarMensajeFlotante === 'function') {
            mostrarMensajeFlotante('⚠️ Esta estación no está disponible en este momento.');
        }
      });
    } else {
      aud.onerror = () => {
    ...
    // ── MEJORA CORREGIDA: Resetear corazón al cambiar de estación y verificar si es favorito ──
    const iconoCorazon = document.querySelector('.icono-btn[title="Favorito"] i');
    if (iconoCorazon) iconoCorazon.style.color = '';
    fetch('backend/favoritos.php?accion=cargar')
        .then(r => r.json())
        .then(data => {
            if (data.ok) {
                const yaEsFav = data.favoritos.some(f => f.Stream_url === url);
                if (iconoCorazon && yaEsFav) iconoCorazon.style.color = '#1cf00c';
            }
        }).catch(() => {});
    ...
    // ── RECUPERADO DE V2: REGISTRO EN BASE DE DATOS DEL HISTORIAL ──
  ...
  // ── CONEXIÓN CON EL BACKEND DE FAVORITOS (EL CORAZÓN) ──
  document.querySelector('.icono-btn[title="Favorito"]').addEventListener('click', async function() {
    ...
    try {
        ...
        if (data.ok) {
            // ── MEJORA CORREGIDA: Evitar duplicados visuales ──
            if (icono.style.color === 'rgb(28, 240, 12)') {
  ...
  document.getElementById('rep-playpause')?.addEventListener('click', () => {
    ...
    if (aud.paused) { aud.play().catch(() => {}); document.getElementById('rep-playpause').innerHTML = '<i class="fas fa-pause"></i>'; } 
    else { aud.pause(); document.getElementById('rep-playpause').innerHTML = '<i class="fas fa-play"></i>'; }
  });
  ...
  // ── RECUPERADO DE V2: Función para sincronizar tamaño real en móviles ──
  function syncSize() {
  ...
  // ── RECUPERADO DE V2: Optimización para Touch/Celulares (Pointer Events) ──
  ...
  // ── RECUPERADO DE V2: ResizeObserver para contenedores dinámicos ──
  ...
  // ── RECUPERADO DE V2: Render diferido para evitar fallos de layout en móvil ──
