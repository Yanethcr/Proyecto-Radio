...
(function () {
  ...
  window._listaActual = [];
  window._indiceActual = -1;

  // historial (lo de antes lo dejamos)
  window._globoHistory = window._globoHistory || [];
  window._globoHistoryIndex = (typeof window._globoHistoryIndex === 'number')
    ? window._globoHistoryIndex : -1;

  let audioActivo = null;
  let estacionActual = null; // Para guardar en favoritos
  // Historial local de estaciones (para prev/next)
  window._globoHistory = window._globoHistory || [];
  window._globoHistoryIndex = (typeof window._globoHistoryIndex === 'number') ? window._globoHistoryIndex : -1;
  ...
  function reproducir(url, nombre, lugar, btn) {
    ...
    //  ACTUALIZAR INDICE EN LISTA
    if (window._listaActual && window._listaActual.length > 0) {
      window._indiceActual = window._listaActual.findIndex(s =>
        (s.url_resolved || s.url) === url
      );
    }

    //  HISTORIAL
    try {
      const last = window._globoHistory[window._globoHistory.length - 1];
      if (!last || last.url !== url) {
        window._globoHistory.push({ url, nombre, lugar });
        window._globoHistoryIndex = window._globoHistory.length - 1;
      }
    } catch (e) {
      window._globoHistory = [{ url, nombre, lugar }];
      window._globoHistoryIndex = 0;
    }
  ...
  // Prev / Next — navegan la lista del panel actual
  function cambiarEstacion(dir) {
    const lista = window._listaActual;
    if (!lista || lista.length === 0) return;

    let base = window._indiceActual;
    if (base < 0 || base >= lista.length) base = dir > 0 ? -1 : lista.length;

    let idx = base + dir;
    if (idx < 0) idx = lista.length - 1;
    if (idx >= lista.length) idx = 0;

    window._indiceActual = idx;
    const st = lista[idx];
    if (!st) return;

    const url = st.url_resolved || st.url;
    if (!url) { cambiarEstacion(dir); return; }

    const nombre = st.name || 'Estación';
    const lugar = document.getElementById('panel-pais')?.textContent || st.country || 'Internacional';
    ...
    document.querySelectorAll('.em-play').forEach(b => b.innerHTML = '<i class="fas fa-play"></i>');
    const items = document.querySelectorAll('.em-item');
    if (items[idx]) {
      items[idx].querySelector('.em-play').innerHTML = '<i class="fas fa-pause"></i>';
      items[idx].scrollIntoView({ behavior: 'smooth', block: 'nearest' });
    }
  }

  document.getElementById('rep-prev').addEventListener('click', () => cambiarEstacion(-1));
  document.getElementById('rep-next').addEventListener('click', () => cambiarEstacion(1));
